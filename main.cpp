...
#define MALLOC(var, type, nelem)					\
  if ((var = (type*)malloc(nelem * sizeof *var)) == NULL) {		\
...
int main(int argc, char *argv[]) {
  ...
  alloc_local = fftw_mpi_local_size_3d_transposed(N, N, Nf, MPI_COMM_WORLD,
						  &local_n0, &local_0_start,
						  &local_n1, &local_1_start);
  MALLOC(U, precision, 2 * alloc_local);
  MALLOC(V, precision, 2 * alloc_local);
  MALLOC(W, precision, 2 * alloc_local);
  MALLOC(U_tmp, precision, 2 * alloc_local);
  MALLOC(V_tmp, precision, 2 * alloc_local);
  MALLOC(W_tmp, precision, 2 * alloc_local);
  MALLOC(CU, precision, 2 * alloc_local);
  MALLOC(CV, precision, 2 * alloc_local);
  MALLOC(CW, precision, 2 * alloc_local);
  ...
  rfftn = fftw_mpi_plan_dft_r2c_3d(
      N, N, N, U, reinterpret_cast<fftw_complex *>(U_hat.data()),
      MPI_COMM_WORLD, FFTW_MPI_TRANSPOSED_OUT);
  irfftn = fftw_mpi_plan_dft_c2r_3d(
      N, N, N, reinterpret_cast<fftw_complex *>(U_hat.data()), U,
  ...
  fftw_mpi_execute_dft_r2c(rfftn, U,
			   reinterpret_cast<fftw_complex *>(U_hat.data()));
  fftw_mpi_execute_dft_r2c(rfftn, V,
			   reinterpret_cast<fftw_complex *>(V_hat.data()));
  fftw_mpi_execute_dft_r2c(rfftn, W,
  ...
  while (t < T - 1e-8) {
	...
	fftw_mpi_execute_dft_c2r(
	    irfftn, reinterpret_cast<fftw_complex *>(U_hat.data()), U);
	fftw_mpi_execute_dft_c2r(
	    irfftn, reinterpret_cast<fftw_complex *>(V_hat.data()), V);
	fftw_mpi_execute_dft_c2r(
	    irfftn, reinterpret_cast<fftw_complex *>(W_hat.data()), W);
	for (k = 0; k < 2 * alloc_local; k++) {
	  U[k] /= tot;
	  V[k] /= tot;
	  W[k] /= tot;
	}
	...
	for (j = 0; j < N; j++)
      ...
      fftw_mpi_execute_dft_c2r(
	  irfftn, reinterpret_cast<fftw_complex *>(curlX.data()), CU);
      fftw_mpi_execute_dft_c2r(
	  irfftn, reinterpret_cast<fftw_complex *>(curlY.data()), CV);
      fftw_mpi_execute_dft_c2r(
	  irfftn, reinterpret_cast<fftw_complex *>(curlZ.data()), CW);
      for (k = 0; k < 2 * alloc_local; k++) {
	CU[k] /= tot;
	CV[k] /= tot;
	CW[k] /= tot;
	...
	for (j = 0; j < N; j++)
      ...
      fftw_mpi_execute_dft_r2c(rfftn, U_tmp,
			       reinterpret_cast<fftw_complex *>(dU.data()));
      fftw_mpi_execute_dft_r2c(rfftn, V_tmp,
			       reinterpret_cast<fftw_complex *>(dV.data()));
      fftw_mpi_execute_dft_r2c(rfftn, W_tmp,
